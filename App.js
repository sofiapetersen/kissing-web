...
app.get('/getallconnections', async (req, res) => {
  try {
      const { data: namesData, error: namesError } = await supabase.from('names').select('*');
      const { data: connectionsData, error: connectionsError } = await supabase.from('connections').select('*');
